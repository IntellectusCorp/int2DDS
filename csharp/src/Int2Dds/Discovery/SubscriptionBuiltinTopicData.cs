    ...
    public sealed class SubscriptionBuiltinTopicData : IDisposable
    {
        ...
        public unsafe string TopicName
        {
            get
            {
                ...
                int ret;
                fixed (byte* p = buf)
                    ret = NativeMethods.int2dds_subscription_builtin_topic_data_get_topic_name(_handle, p, (UIntPtr)buf.Length, out size);
        ...
        public unsafe string TypeName
        {
            get
            {
                ...
                int ret;
                fixed (byte* p = buf)
                    ret = NativeMethods.int2dds_subscription_builtin_topic_data_get_type_name(_handle, p, (UIntPtr)buf.Length, out size);
                if (ret == ReturnCode.NoData || (uint)size == 0) return string.Empty;
                if (ret == ReturnCode.BufferTooSmall)
                {
                    buf = new byte[(int)(uint)size];
