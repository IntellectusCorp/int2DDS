...
pub struct RustOptions {
    ...
    /// Default extensibility for types without explicit @extensibility in IDL.
    /// None = omit extensibility annotation (plain CDR/Final).
    pub default_extensibility: Option<ExtensibilityKind>,
}

impl Default for RustOptions {
    fn default() -> Self {
        Self { crate_path: "int2dds".to_string(), default_extensibility: None }
    }
}
...
impl<'a> RustGen<'a> {
    fn emit_file(&mut self, idl_filename: &str) {
        ...
        let needs_wchar = self.needs_type_import(|ty| matches!(ty, ResolvedType::WChar));
        let needs_wstring = self.needs_type_import(|ty| matches!(ty, ResolvedType::WString { .. }));
        if needs_wchar && needs_wstring {
            self.line(&format!(
            ...
            self.line(&format!(
                "use {}::serialize::core::WChar;",
                self.opts.crate_path
            ));
        } else if needs_wstring {
            self.line(&format!(
                "use {}::serialize::core::WString;",
                self.opts.crate_path
            ));
        }
    ...
    fn needs_type_import(&self, predicate: impl Fn(&ResolvedType) -> bool) -> bool {
        self.model.structs.iter().any(|s| {
            s.members.iter().any(|m| self.type_matches_recursive(&m.resolved_type, &predicate))
        }) || self.model.unions.iter().any(|u| {
            u.cases.iter().any(|c| self.type_matches_recursive(&c.member.resolved_type, &predicate))
                || u.default_case.as_ref().is_some_and(|dc| self.type_matches_recursive(&dc.resolved_type, &predicate))
        })
    }

    fn type_matches_recursive(&self, ty: &ResolvedType, predicate: &impl Fn(&ResolvedType) -> bool) -> bool {
        if predicate(ty) {
            return true;
        }
        match ty {
            ResolvedType::Sequence { element, .. } | ResolvedType::Array { element, .. } => {
                self.type_matches_recursive(element, predicate)
            }
            ResolvedType::Map { key, value, .. } => {
                self.type_matches_recursive(key, predicate) || self.type_matches_recursive(value, predicate)
            }
            _ => false,
        }
    }
    ...
    fn emit_bitmask(&mut self, b: &ResolvedBitmask) {
        ...
        self.line("#[derive(DdsType)]");
        self.line(&format!(
            "#[dds_type(crate_path = \"{}\", bitmask, bit_bound = {})]",
            self.opts.crate_path, b.bit_bound
        ));
        self.line(&format!("pub enum {} {{", rust_name));
        self.indent += 1;
        for flag in &b.flags {
    ...
    fn emit_union(&mut self, u: &ResolvedUnion) {
        ...
        if let Some(dc) = &u.default_case {
            ...
            // Mark with #[dds(union_default)] so derive macro generates catch-all match.
            self.line("#[dds(union_default)]");
    ...
    fn emit_struct(&mut self, s: &ResolvedStruct) {
        ...
        // IDL-specified extensibility takes priority, then CLI default
        let effective_ext = s.extensibility.or(self.opts.default_extensibility);
        if let Some(ext) = effective_ext {
            match ext {
...
mod tests {
    ...
    fn test_hello_world_codegen() {
        ...
        assert!(code.contains("#[dds_type(crate_path = \"int2dds\", extensibility = \"Appendable\")]"));
    ...
    fn test_bitmask_codegen() {
        ...
        assert!(code.contains("#[dds_type(crate_path = \"int2dds\", bitmask, bit_bound = 8)]"));
    ...
    fn test_bitset_codegen() {
        ...
        assert!(code.contains("#[dds_type(crate_path = \"int2dds\", bitset)]"));
