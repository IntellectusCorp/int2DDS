...
use crate::codegen::{
    ...
    generate_enum_xcdr_serialize_impl, generate_field_deserialization,
    ...
    parse_field_attributes, parse_repr_attribute, KeyFieldInfo, MultiKeyFieldInfo,
};
...
pub fn derive_dds_type_impl(input: &DeriveInput) -> proc_macro2::TokenStream {
    ...
    match &input.data {
        Data::Struct(data) => {
            ...
            derive_struct_impl(input, name, fields, &type_config, crate_path)
        }
        Data::Enum(data) => {
            let variants = &data.variants;
            derive_enum_impl(input, name, variants, &type_config, crate_path)
        }
...
fn generate_enum_type_support_impl(
...
fn parse_dds_type_attributes(input: &DeriveInput) -> DdsTypeConfig {
