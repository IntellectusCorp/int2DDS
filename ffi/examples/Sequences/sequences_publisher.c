/**
 * int2dds FFI Sequences Type Publisher
 *
 * Topic: SequencesTestTopic
 * Type:  SequencesType
 */
...
int main(int argc, char *argv[]) {
    ...
    printf("=== int2dds Sequences Publisher ===\n");
    printf("Topic: SequencesTestTopic\n");
    printf("Type:  SequencesType (APPENDABLE)\n");
    ...
    ret = int2dds_create_participant(factory, "sequences_publisher", domain_id, &participant);
    ...
    {
        Int2DdsTypeInfo *type_info = SequencesType_type_info();
        ret = int2dds_create_topic_with_type_info(participant, "SequencesTestTopic",
    ...
    /* Stack-allocated sequence buffers */
    bool bool_buf[3] = {true, false, true};
    uint8_t byte_buf[3] = {0, 127, 255};
    int32_t i32_buf[16];
    float f32_buf[3] = {1.1f, 2.2f, 3.3f};
    double f64_buf[3] = {1.11, 2.22, 3.33};
    char string_buf[16][257];
    int32_t bounded_buf[5];
    ...
    while (g_running) {
        ...
        data.bool_seq.data = bool_buf;
        data.bool_seq.length = 3;

        /* byte_seq: {0, 127, 255} */
        data.byte_seq.data = byte_buf;
        data.byte_seq.length = 3;

        /* i32_seq: {0, 1, 2, ..., sample_index % 10} */
        uint32_t i32_len = (sample_index % 10) + 1;
        for (uint32_t j = 0; j < i32_len; j++) i32_buf[j] = (int32_t)j;
        data.i32_seq.data = i32_buf;
        data.i32_seq.length = i32_len;

        /* f32_seq, f64_seq: fixed values */
        data.f32_seq.data = f32_buf;
        data.f32_seq.length = 3;
        data.f64_seq.data = f64_buf;
        data.f64_seq.length = 3;

        /* string_seq: growing from 1 to 3, cycling */
        {
            uint32_t str_len = (sample_index % 3) + 1;
            for (uint32_t j = 0; j < str_len; j++) {
                snprintf(string_buf[j], sizeof(string_buf[j]), "item_%u", j);
            }
            data.string_seq.data = string_buf;
            data.string_seq.length = str_len;
        }

        /* bounded_seq: {1, 2, ..., len} where len cycles 1..5 */
        uint32_t bounded_len = (sample_index % 5) + 1;
        for (uint32_t j = 0; j < bounded_len; j++) bounded_buf[j] = (int32_t)(j + 1);
        data.bounded_seq.length = bounded_len;
        memcpy(data.bounded_seq.data, bounded_buf, bounded_len * sizeof(int32_t));

        size_t serialized_len = SequencesType_serialize_cdr(&data, buf, sizeof(buf), use_xcdr2);
        if (serialized_len == 0) { fprintf(stderr, "Serialization failed\n"); sample_index++; continue; }

        size_t key_len = SequencesType_serialize_key(&data, key_buf, sizeof(key_buf));

        ret = int2dds_write_serialized(writer, buf, serialized_len, key_buf, key_len);
        if (ret != INT2DDS_RET_OK) {
            fprintf(stderr, "Write failed: %d\n", ret);
        } else {
            printf("[%u] Sent SequencesType (i32_seq.len=%u, string_seq.len=%u, bounded_seq.len=%u)\n",
                   sample_index, data.i32_seq.length, data.string_seq.length,
                   data.bounded_seq.length);
        }
