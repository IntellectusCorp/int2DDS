/**
 * int2dds FFI Sequence_Enum Type Subscriber
 *
...
static void on_data_available(Int2DdsDataReader *reader, void *user_ctx) {
    ...
    while ((ret = int2dds_take_serialized(reader, recv_buf, sizeof(recv_buf),
        ...
        if (SequenceEnumType_deserialize_cdr(recv_buf, actual_size, &data)) {
            printf("Received SequenceEnumType (color_seq.len=%u: [", data.color_seq.length);
            for (uint32_t j = 0; j < data.color_seq.length; j++) {
                if (j > 0) printf(", ");
                printf("%s", color_to_string(data.color_seq.data[j]));
            }
            printf("])\n");
            ctx->received_count++;
...
int main(int argc, char *argv[]) {
    ...
    printf("=== int2dds Sequence_Enum Subscriber ===\n");
    ...
    ret = int2dds_create_participant(factory, "sequence_enum_subscriber", domain_id, &participant);
