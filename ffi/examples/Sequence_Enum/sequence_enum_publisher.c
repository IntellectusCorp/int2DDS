/**
 * int2dds FFI Sequence_Enum Type Publisher
 *
 * Topic: SequenceEnumTestTopic
...
int main(int argc, char *argv[]) {
    ...
    printf("=== int2dds Sequence_Enum Publisher ===\n");
    printf("Topic: SequenceEnumTestTopic\n");
    printf("Type:  SequenceEnumType (APPENDABLE)\n");
    ...
    ret = int2dds_create_participant(factory, "sequence_enum_publisher", domain_id, &participant);
    ...
    {
        Int2DdsTypeInfo *type_info = SequenceEnumType_type_info();
        ret = int2dds_create_topic_with_type_info(participant, "SequenceEnumTestTopic",
    ...
    static const Color all_colors[] = {COLOR_RED, COLOR_GREEN, COLOR_BLUE, COLOR_YELLOW, COLOR_PURPLE};
    Color color_buf[10];

    SequenceEnumType data;
    uint8_t buf[512];
    uint8_t key_buf[16];
    ...
    while (g_running) {
        ...
        /* Growing cycle of Color values */
        uint32_t color_len = (sample_index % 5) + 1;
        for (uint32_t j = 0; j < color_len; j++) {
            color_buf[j] = all_colors[(sample_index + j) % 5];
        }
        data.color_seq.data = color_buf;
        data.color_seq.length = color_len;

        size_t serialized_len = SequenceEnumType_serialize_cdr(&data, buf, sizeof(buf), use_xcdr2);
        if (serialized_len == 0) { fprintf(stderr, "Serialization failed\n"); sample_index++; continue; }

        size_t key_len = SequenceEnumType_serialize_key(&data, key_buf, sizeof(key_buf));

        ret = int2dds_write_serialized(writer, buf, serialized_len, key_buf, key_len);
        if (ret != INT2DDS_RET_OK) {
            fprintf(stderr, "Write failed: %d\n", ret);
        } else {
            printf("[%u] Sent SequenceEnumType (color_seq.len=%u: [", sample_index, color_len);
            for (uint32_t j = 0; j < color_len; j++) {
                if (j > 0) printf(", ");
                printf("%s", color_to_string(color_buf[j]));
            }
            printf("])\n");
        }
