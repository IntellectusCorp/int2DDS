/**
 * int2dds FFI Enum Type Publisher
 *
 * Topic: EnumTestTopic
 * Type:  EnumType
 */
...
int main(int argc, char *argv[]) {
    ...
    ret = int2dds_create_participant(factory, "enum_publisher", domain_id, &participant);
    ...
    static const Color colors[] = {COLOR_RED, COLOR_GREEN, COLOR_BLUE, COLOR_YELLOW, COLOR_PURPLE};
    static const StatusKind statuses[] = {STATUS_KIND_UNKNOWN, STATUS_KIND_ACTIVE, STATUS_KIND_INACTIVE, STATUS_KIND_PENDING};
    ...
    while (g_running) {
        ...
        data.id = 1;
        data.color = colors[sample_index % 5];
        data.status = statuses[sample_index % 4];

        size_t serialized_len = EnumType_serialize_cdr(&data, buf, sizeof(buf), use_xcdr2);
        if (serialized_len == 0) { fprintf(stderr, "Serialization failed\n"); sample_index++; continue; }

        size_t key_len = EnumType_serialize_key(&data, key_buf, sizeof(key_buf));

        ret = int2dds_write_serialized(writer, buf, serialized_len, key_buf, key_len);
        if (ret != INT2DDS_RET_OK) {
            fprintf(stderr, "Write failed: %d\n", ret);
        } else {
            printf("[%u] Sent EnumType (color=%s, status=%s)\n",
                   sample_index, color_to_string(data.color), status_to_string(data.status));
        }
