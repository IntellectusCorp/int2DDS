/**
 * int2dds FFI Nested_Structs Type Subscriber
...
static void on_data_available(Int2DdsDataReader *reader, void *user_ctx) {
    ...
    /* Pre-allocate buffer for struct_seq.
     * Static to avoid large stack allocation in callback thread. */
    static InnerStruct seq_buf[64];
    ...
    while ((ret = int2dds_take_serialized(reader, recv_buf, sizeof(recv_buf),
        ...
        if (NestedStructType_deserialize_cdr(recv_buf, actual_size, &data)) {
            printf("Received NestedStructType (simple={x=%d,y=%d,name=\"%s\"}, "
                   "deep={level=%d,deep={value=%d,desc=\"%s\"}}, struct_seq.len=%u)\n",
                   data.simple_nested.x, data.simple_nested.y, data.simple_nested.name,
                   data.deep_nested.level, data.deep_nested.deep.value,
                   data.deep_nested.deep.description,
                   data.struct_seq.length);

            for (uint32_t j = 0; j < data.struct_seq.length; j++) {
                printf("  seq[%u]: x=%d, y=%d, name=\"%s\"\n",
                       j, data.struct_seq.data[j].x,
                       data.struct_seq.data[j].y,
                       data.struct_seq.data[j].name);
            }
...
int main(int argc, char *argv[]) {
    ...
    printf("=== int2dds Nested_Structs Subscriber ===\n");
    ...
    ret = int2dds_create_participant(factory, "nested_structs_subscriber", domain_id, &participant);
