/**
 * int2dds FFI Strings Type Subscriber
 *
...
static void on_data_available(Int2DdsDataReader *reader, void *user_ctx) {
    ...
    while ((ret = int2dds_take_serialized(reader, recv_buf, sizeof(recv_buf),
        ...
        if (StringsType_deserialize_cdr(recv_buf, actual_size, &data)) {
            printf("Received StringsType (unbounded=\"%s\", bounded=\"%s\")\n",
                   data.unbounded_str, data.bounded_str);
            ctx->received_count++;
...
int main(int argc, char *argv[]) {
    ...
    printf("=== int2dds Strings Subscriber ===\n");
    ...
    ret = int2dds_create_participant(factory, "strings_subscriber", domain_id, &participant);
