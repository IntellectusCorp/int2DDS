/**
 * int2dds FFI Floats Type Subscriber
 *
...
static void on_data_available(Int2DdsDataReader *reader, void *user_ctx) {
    ...
    while ((ret = int2dds_take_serialized(reader, recv_buf, sizeof(recv_buf),
        ...
        if (FloatsType_deserialize_cdr(recv_buf, actual_size, &data)) {
            printf("Received FloatsType (f32=%.6f, f64=%.15f)\n",
                   data.f32_val, data.f64_val);
            ctx->received_count++;
...
int main(int argc, char *argv[]) {
    ...
    printf("=== int2dds Floats Subscriber ===\n");
    ...
    ret = int2dds_create_participant(factory, "floats_subscriber", domain_id, &participant);
