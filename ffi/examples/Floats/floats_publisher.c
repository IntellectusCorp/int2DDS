/**
 * int2dds FFI Floats Type Publisher
 *
 * Topic: FloatsTestTopic
 * Type:  FloatsType
 */
...
int main(int argc, char *argv[]) {
    ...
    printf("=== int2dds Floats Publisher ===\n");
    printf("Topic: FloatsTestTopic\n");
    printf("Type:  FloatsType (APPENDABLE)\n");
    ...
    ret = int2dds_create_participant(factory, "floats_publisher", domain_id, &participant);
    ...
    {
        Int2DdsTypeInfo *type_info = FloatsType_type_info();
        ret = int2dds_create_topic_with_type_info(participant, "FloatsTestTopic",
    ...
    while (g_running) {
        ...
        data.id = 1;
        data.f32_val = 3.14159f * (float)sample_index;
        data.f64_val = 2.718281828459045 * (double)sample_index;

        size_t serialized_len = FloatsType_serialize_cdr(&data, buf, sizeof(buf), use_xcdr2);
        if (serialized_len == 0) { fprintf(stderr, "Serialization failed\n"); sample_index++; continue; }

        size_t key_len = FloatsType_serialize_key(&data, key_buf, sizeof(key_buf));

        ret = int2dds_write_serialized(writer, buf, serialized_len, key_buf, key_len);
        if (ret != INT2DDS_RET_OK) {
            fprintf(stderr, "Write failed: %d\n", ret);
        } else {
            printf("[%u] Sent FloatsType (f32=%.6f, f64=%.15f)\n",
                   sample_index, data.f32_val, data.f64_val);
        }
