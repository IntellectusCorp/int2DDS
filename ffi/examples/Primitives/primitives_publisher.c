 ...
 *
 * Publishes PrimitivesType data for FastDDS CDR interop testing.
 * Uses APPENDABLE extensibility for wire-compatibility with FastDDS.
 *
...
int main(int argc, char *argv[]) {
    ...
    while (g_running) {
        ...
        /* Fill data - same pattern as FastDDS publisher */
        data.id = 1;
        ...
        /* Serialize */
        size_t serialized_len = PrimitivesType_serialize_cdr(&data, buf, sizeof(buf));
        if (serialized_len == 0) {
