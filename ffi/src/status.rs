...
mod tests {
    ...
    #[test]
    fn test_publication_matched_status_conversion() {
        let rust_status = PublicationMatchedStatus {
            total_count: 10,
            total_count_change: 2,
            current_count: 5,
            current_count_change: 1,
            last_subscription_handle: InstanceHandle::new([1; 16]),
        };

        let ffi_status: Int2DdsPublicationMatchedStatus = (&rust_status).into();

        assert_eq!(ffi_status.total_count, 10);
        assert_eq!(ffi_status.total_count_change, 2);
        assert_eq!(ffi_status.current_count, 5);
        assert_eq!(ffi_status.current_count_change, 1);
        assert_eq!(ffi_status.last_subscription_handle, [1; 16]);
    }

    #[test]
    fn test_subscription_matched_status_conversion() {
        let rust_status = SubscriptionMatchedStatus {
            total_count: 8,
            total_count_change: 1,
            current_count: 3,
            current_count_change: -1,
            last_publication_handle: InstanceHandle::new([2; 16]),
        };

        let ffi_status: Int2DdsSubscriptionMatchedStatus = (&rust_status).into();

        assert_eq!(ffi_status.total_count, 8);
        assert_eq!(ffi_status.total_count_change, 1);
        assert_eq!(ffi_status.current_count, 3);
        assert_eq!(ffi_status.current_count_change, -1);
        assert_eq!(ffi_status.last_publication_handle, [2; 16]);
    }
}
