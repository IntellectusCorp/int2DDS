...
impl<Foo: 'static + Clone + Debug> DataReaderHistoryCache<Foo> {
    ...
    fn update_instance_state(&self, cache_change: &CacheChange) -> DdsResult<()> {
        ...
        if cache_change.kind() == ChangeKind::Alive
            ...
            || cache_change.kind() == ChangeKind::NotAliveDisposed
        {
            if !self.is_writer_owner_of_instance(
            ...
            let new_state = match cache_change.kind() {
        ...
        } else if cache_change.kind() == ChangeKind::NotAliveUnregistered {
...
mod tests {
    ...
    mod ownership_qos {
        ...
        use crate::infrastructure::{
            qos_policy::{DeadlineQosPolicy, LivelinessQosPolicy, LivelinessQosPolicyKind},
            wait_set::WaitSet,
        };
        ...
        #[test]
        fn test_history_cache_owner_reclaim_ownership_again_after_deadline_missed() {
            let reader_qos = DataReaderQos {
                ownership: OwnershipQosPolicy { kind: OwnershipQosPolicyKind::Exclusive },
                deadline: DeadlineQosPolicy { period: Duration::from_millis(1000) },
                ..Default::default()
            ...
            // Now writer B is the owner again
            let is_owner_a = datareader_cache
                .lock()
                .unwrap()
                .is_writer_owner_of_instance(writer_a.clone(), instance_handle)
        ...
        #[test]
        fn test_history_cache_owner_revoked_when_liveliness_lost() {
            let reader_qos = DataReaderQos {
                ownership: OwnershipQosPolicy { kind: OwnershipQosPolicyKind::Exclusive },
            ...
            let data_writer = publisher
                .create_datawriter::<ShapeType>(&topic, writer_qos, None, StatusMask::default())
                .unwrap();
            ...
            let writer_guid = data_writer.get_instance_handle().unwrap().to_guid();
            ...
            data_writer.write(&data, InstanceHandle::NIL).unwrap(); // Published and asserted liveliness
            ...
            let instance_handle = datareader_cache
                .lock()
                .unwrap()
                .get_instance_map()
                .lock()
            ...
            let is_owner = datareader_cache
                .lock()
                .unwrap()
                .is_writer_owner_of_instance(writer_guid, instance_handle)
                .unwrap();

            assert!(is_owner);

            // does nothing and waits for liveliness to be lost
            thread::sleep(std::time::Duration::from_millis(1500));
