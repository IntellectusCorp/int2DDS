...
pub enum PublishModeQosPolicyKind {
    /// `write()` sends inline on the user thread. Default — latency-optimal
    /// for single-sample workloads since the call performs the wire send
    /// without a thread handoff.
    #[default]
    Synchronous,
