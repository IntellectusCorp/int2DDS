...
mod tests {
    ...
    // The serialized payload the peer reads back out of a one-DATA datagram.
    ...
    fn a_non_shm_destination_carries_the_payload() {
        ...
        // The submessage body is padded to a 4-byte boundary, so compare the prefix.
        // The 24-byte descriptor needs no padding, which is why the shm test can compare
        // the whole thing.
        let payload = data_payload_of(&buffer);
    ...
    // A sample past the message-size budget is what the sender would otherwise split into
    // DATA_FRAG. Bound for shm it is a 24-byte descriptor, so one DATA carries it whole and
...
impl MessageCreator {
    ...
    /// `to_shm` says the caller already picked an SHM locator for this datagram, so a
    /// slot-backed change may travel as its descriptor instead of its bytes. Only the
    /// caller knows the destination: nothing below here does.
    #[allow(clippy::too_many_arguments)]
    ...
    fn build_data_submessage<'a>(
        ...
        // The descriptor is built here, so it cannot be `Borrowed`: the submessage
        // outlives this frame. 24 bytes owned, against the payload copy it replaces.
