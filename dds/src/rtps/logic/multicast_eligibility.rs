...
pub(crate) fn group_can_carry_change(change: &CacheChange, group_sent_sn: SequenceNumber) -> bool {
    ...
    sample_allows_multicast(change)
}

/// Whether a sample may go out as one datagram shared by a whole group.
///
/// Only a first transmission ever asks: a repair answers one reader's ACKNACK or
...
mod tests {
    ...
    fn a_reader_without_a_group_is_not_selected_for_an_arriving_datagram() {
        ...
        let selected = readers_listening_on(
            vec![
                (
                    "on_arrival_group",
                    subscription_with_groups(std::slice::from_ref(&arrival_group)),
                ),
    ...
    fn every_reader_on_the_arriving_group_is_selected() {
        ...
        let selected = readers_listening_on(
            vec![
                ("first", subscription_with_groups(std::slice::from_ref(&arrival_group))),
                ("second", subscription_with_groups(std::slice::from_ref(&arrival_group))),
            ],
            &arrival_group,
        );
    ...
    fn reader_with_reachable_group_is_eligible() {
        ...
        let subscription = subscription_with_groups(std::slice::from_ref(&group));
    ...
    fn target_is_keyed_by_its_first_reachable_group() {
        ...
        let only_second = subscription_with_groups(std::slice::from_ref(&reachable));
    ...
    fn plain_first_sample_allows_multicast() {
        assert!(sample_allows_multicast(&plain_change()));
    }
