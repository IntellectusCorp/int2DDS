...
pub struct RingBufferHeader {
    ...
    pub buffer_size: u32,
    /// Reserve position (atomically updated by multiple writers via CAS to reserve space)
    pub reserve_pos: AtomicU64,
    /// Commit position (updated after data is fully written, readers use this)
    pub commit_pos: AtomicU64,
...
impl RingBufferHeader {
    ...
    pub fn init(&mut self, buffer_size: u32, max_message_size: u32) {
        ...
        self.reserve_pos = AtomicU64::new(0);
        self.commit_pos = AtomicU64::new(0);
...
/// Maximum spin iterations waiting for commit_pos to catch up
const MAX_COMMIT_SPIN: usize = 10000;
...
impl RingBufferWriter {
    ...
    /// Write a message to the ring buffer using 2-phase commit
    ///
    /// Multiple writers can safely call this method concurrently.
    /// Phase 1: Reserve space atomically using CAS on reserve_pos
    /// Phase 2: Write data, then update commit_pos in order
    ///
    /// Readers only see commit_pos, ensuring they never read incomplete data.
    pub fn write(&mut self, data: &[u8]) -> Result<usize, RingBufferError> {
        ...
        // Phase 1: CAS loop to reserve space
        let mut retries = 0;
        let (reserved_pos, reserved_size) = loop {
            if retries >= MAX_CAS_RETRIES {
                return Err(RingBufferError::BufferFull);
            }

            let current_pos = header.reserve_pos.load(Ordering::SeqCst);
            ...
            if total_size > remaining_space {
                // Need to insert wrap marker and wrap around
                if remaining_space >= MessageHeader::SIZE {
                    // Reserve space for wrap marker + actual message at beginning
                    let wrap_reserve = remaining_space as u64 + total_size as u64;
                    match header.reserve_pos.compare_exchange(
                        current_pos,
                        current_pos + wrap_reserve,
                        Ordering::SeqCst,
                        Ordering::SeqCst,
                    ) {
                        Ok(_) => {
                            // We reserved: [wrap_marker at current_pos] + [message at 0]
                            // Write wrap marker immediately
                            ...
                            // Return position 0 (after wrap) for actual message
                            break (
                                current_pos + remaining_space as u64,
                                remaining_space + total_size,
                            );
                        }
                        Err(_) => {
                            retries += 1;
                    ...
                    // Not enough space even for wrap marker header, skip remaining bytes
                    let skip_size = remaining_space as u64 + total_size as u64;
                    match header.reserve_pos.compare_exchange(
                        current_pos,
            ...
            // Normal case: reserve space for message
            ...
            match header.reserve_pos.compare_exchange(
                ...
                Ok(_) => break (current_pos, total_size), // Successfully reserved space
                Err(_) => {
                    retries += 1;
                    continue;
                }
        ...
        // Write the data to reserved space
        ...
        // Memory fence to ensure all data writes are visible before updating commit_pos
        std::sync::atomic::fence(Ordering::SeqCst);

        // Phase 2: Update commit_pos in order
        // We must wait until commit_pos catches up to our reserved_pos
        // This ensures commits happen in reservation order
        let expected_commit = reserved_pos - (reserved_size - total_size) as u64;
        // For wrap case: expected_commit is the position before wrap marker
        // For normal case: expected_commit == reserved_pos
        let commit_wait_pos = reserved_pos - (reserved_size - total_size) as u64;

        let mut spin_count = 0;
        loop {
            let current_commit = header.commit_pos.load(Ordering::SeqCst);
            if current_commit == commit_wait_pos {
                // Our turn to commit
                let new_commit = commit_wait_pos + reserved_size as u64;
                header.commit_pos.store(new_commit, Ordering::SeqCst);
                break;
            }

            spin_count += 1;
            if spin_count >= MAX_COMMIT_SPIN {
                // Timeout waiting for previous writers - this shouldn't happen normally
                // but we need to handle it to avoid deadlock
                warn!(
                    "[RingBuffer] Commit timeout: waiting for commit_pos {} to reach {}, current reserve_pos {}",
                    current_commit, commit_wait_pos, header.reserve_pos.load(Ordering::SeqCst)
                );
                // Force commit to prevent deadlock (may cause reader to see stale data briefly)
                header.commit_pos.store(commit_wait_pos + reserved_size as u64, Ordering::SeqCst);
                break;
            }
...
/// Readers only see data that has been fully committed (commit_pos).
pub struct RingBufferReader {
    /// Pointer to the header in shared memory (read-only access to commit_pos)
...
impl RingBufferReader {
    ...
    pub unsafe fn new(header: *const RingBufferHeader, data: *const u8) -> Self {
        let buffer_size = (*header).buffer_size as usize;
        // Initialize local_read_pos to current commit_pos to only receive new messages
        // We use commit_pos (not reserve_pos) to ensure we only see fully written data
        let local_read_pos = (*header).commit_pos.load(Ordering::SeqCst);
        Self { header, data, buffer_size, local_read_pos, last_sequence: 0 }
    }

    /// Read a message from the ring buffer
    ///
    /// Only reads data that has been fully committed by writers.
    pub fn read(&mut self, buffer: &mut [u8]) -> Result<Option<(usize, u64)>, RingBufferError> {
        let header = unsafe { &*self.header };

        // Use commit_pos (not reserve_pos) to ensure we only read fully written data
        let commit_pos = header.commit_pos.load(Ordering::SeqCst);
        let read_pos = self.local_read_pos;

        // Check if buffer is empty (no new committed data since our last read)
        if commit_pos == read_pos {
            return Ok(None);
        }

        // Check if we're too far behind (data was overwritten)
        // Use reserve_pos to check for overwrite since that's the actual write frontier
        let reserve_pos = header.reserve_pos.load(Ordering::SeqCst);
        let distance = reserve_pos.saturating_sub(read_pos);

        if distance > self.buffer_size as u64 {
            // Skip to a safe position (current commit_pos)
            warn!(
                "[RingBuffer] Reader too far behind, skipping from {} to {} ({} bytes lost)",
                read_pos,
                commit_pos,
                commit_pos.saturating_sub(read_pos)
            );
            self.local_read_pos = commit_pos;
        ...
        if total_len == 0 || total_len > self.buffer_size {
            // Corrupted header, skip to current commit position
            warn!(
