...
//!
//! Per instance, a reader sees at most one sample every `minimum_separation`. A sample that
//! arrives inside the separation window is held instead of dropped, and the most recent held
//! sample is delivered by a one-shot timer once the window elapses. This keeps the last value
//! flowing even after the writer stops, which a drop-on-arrival filter would lose.
...
impl TimeBasedFilter {
    ...
    // Decide whether an ALIVE sample passes now or is held. `min_separation_nanos` is the
    // QoS minimum_separation in nanoseconds; `now` is the reader's current time.
    pub(crate) fn on_alive_sample(
        ...
        let Some(state) = instances.get_mut(&instance_handle) else {
            ...
            return FilterOutcome::Deliver(change);
        };

        // A delayed delivery is already scheduled: keep only the newest sample and let the
        // timer deliver it. Checked before the window test so a sample arriving at the window
        // boundary cannot race past the pending one and be delivered alongside it.
...
mod tests {
    ...
    fn alive(seq: i64, instance: u8) -> CacheChange {
        CacheChange::new(
            ChangeKind::Alive,
            Guid::UNKNOWN,
