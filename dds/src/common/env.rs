...
pub fn init_from_env() {
    ...
    // - INT2DDS_SHM_POOL_SIZE: Set the shared memory pool per participant for shm zero-copy (bytes, or K/M/G suffix; power of two >= 4K) - Default: 32M
...
/// Set the shm zero-copy pool size (bytes) via environment variable
pub fn set_shm_pool_size(size: u64) {
    log::info!("Environment variable set: INT2DDS_SHM_POOL_SIZE = {}", size);
    unsafe { std::env::set_var("INT2DDS_SHM_POOL_SIZE", size.to_string()) };
}
