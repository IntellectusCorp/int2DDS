...
#[test]
fn test_ownership_revoked_when_deadline_missed_no_key() {
    let domain_id = next_domain_id();
    let factory = DomainParticipantFactory::get_instance();
    let participant = factory
        .create_participant(domain_id, DomainParticipantQos::default(), None, StatusMask::default())
    ...
    let weaker_data_writer =
        create_nokey_datawriter(&participant, PublisherQos::default(), weaker_writer_qos);
    let stronger_data_writer =
        create_nokey_datawriter(&participant, PublisherQos::default(), stronger_writer_qos);
    ...
    stronger_data_writer.write(&NoKeyDataType::default(), InstanceHandle::NIL).unwrap();
    wait_for_reader_status(&data_reader, StatusMask::DATA_AVAILABLE, Duration::from_seconds(5))
        .unwrap();
    ...
    weaker_data_writer.write(&NoKeyDataType::default(), InstanceHandle::NIL).unwrap();
    let res = wait_for_reader_status(
        &data_reader,
        StatusMask::DATA_AVAILABLE,
        Duration::from_millis(1500),
...
#[test]
fn test_ownership_revoked_when_liveliness_lost_no_key() {
    let domain_id = next_domain_id();
    let factory = DomainParticipantFactory::get_instance();
    let participant = factory
        .create_participant(domain_id, DomainParticipantQos::default(), None, StatusMask::default())
    ...
    let data_reader = create_nokey_datareader(&participant, SubscriberQos::default(), reader_qos);
    ...
    // Stronger writer's data should be received
    stronger_data_writer.write(&NoKeyDataType::new(1), InstanceHandle::NIL).unwrap();
    wait_for_reader_status(&data_reader, StatusMask::DATA_AVAILABLE, Duration::from_millis(1000))
        .unwrap();
    ...
    weaker_data_writer.write(&NoKeyDataType::new(2), InstanceHandle::NIL).unwrap();
    ...
    // Now weaker writer should become owner
    debug!("Weaker writer trying to write data...");
    weaker_data_writer.write(&NoKeyDataType::new(3), InstanceHandle::NIL).unwrap();
    debug!("Weaker writer wrote data after stronger writer lost liveliness.");
