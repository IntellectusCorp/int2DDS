...
fn reliable_pubsub_roundtrip(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("--reliable_pubsub");

    group.bench_function("reliable_pubsub", |b| {
        b.iter(|| {
            // write
...
criterion_group!(benches, reliable_pubsub_roundtrip);
