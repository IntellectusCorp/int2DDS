...
const HEADER: &str = "// @generated by `cargo run -p int2dds-java --bin gen-jni`\n\
     ...
     \x20   }\n\n\
     \x20   /** Native base address of a direct ByteBuffer, or 0 if not direct. */\n\
     \x20   static long directBufferAddress(java.nio.ByteBuffer buf) {\n\
     \x20       if (buf == null || !buf.isDirect()) return 0L;\n\
     \x20       // ofBuffer starts at the buffer's position; JNI's GetDirectBufferAddress does not.\n\
     \x20       return MemorySegment.ofBuffer(buf).address() - buf.position();\n\
     \x20   }\n\n";
...
mod tests {
    ...
    fn the_real_surface_renders_every_generatable_function_without_panicking() {
        ...
        assert_eq!(
            ...
            "one MethodHandle per generatable function"
        );
        assert!(
